...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Account created! Welcome to RouteX 🚀')
      navigate('/')
    } catch (err) {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        setAuth(res.data.user, res.data.token)
        toast.success('Account ready! Welcome to RouteX 🚀')
        navigate('/')
      } catch (err) {
    ...
    <div style={{ minHeight: '100vh', background: '#000', display: 'flex' }}>
      ...
      <div className="hide-mobile" style={{ flex: 1, position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ position: 'relative', textAlign: 'center', padding: 40 }}>
          ...
          <p style={{ fontSize: 18, color: '#a1a1aa', maxWidth: 280, margin: '0 auto' }}>Bus • Train • Flight<br />All in one platform.</p>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
          ...
          <h1 className="font-display" style={{ fontSize: 28, fontWeight: 700, marginBottom: 6 }}>Create Account</h1>
          <p style={{ color: '#71717a', fontSize: 14, marginBottom: 32 }}>Join RouteX and start booking</p>

          {/* Google sign-up — shown first for quick onboarding */}
          <button
            onClick={() => { setGoogleLoading(true); googleLogin() }}
            disabled={googleLoading}
            style={{ width: '100%', padding: '12px 0', borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#a1a1aa', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.15s', marginBottom: 20, opacity: googleLoading ? 0.6 : 1 }}
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <input className="input-flat" placeholder="Full Name *" value={form.name}
              onChange={e => setForm({ ...form, name: e.target.value })} />
            <input className="input-flat" type="email" placeholder="Email *" value={form.email}
            ...
            <input className="input-flat" type="password" placeholder="Password * (min 6 chars)" value={form.password}
              onChange={e => setForm({ ...form, password: e.target.value })} />
            <input className="input-flat" type="password" placeholder="Confirm Password *" value={form.confirmPassword}
          ...
          <p style={{ textAlign: 'center', marginTop: 20, fontSize: 13, color: '#71717a' }}>
            Already have an account? <Link to="/login" style={{ color: '#ef233c', fontWeight: 600 }}>Sign In</Link>
          </p>
        </motion.div>
      </div>
    </div>
